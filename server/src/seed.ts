...
export const seedDatabase = async () => {
    try {
        ...
        const users = await User.create([
            { username: 'admin', password: process.env.DEFAULT_ADMIN_PASSWORD || 'change_me_admin', role: 'admin', syncsRemaining: 999, maxWebsites: 999 },
            { username: 'garg', password: process.env.DEFAULT_USER_PASSWORD || 'change_me_user', role: 'user', syncsRemaining: 30, maxWebsites: 1 },
            { username: 'test', password: process.env.DEFAULT_USER_PASSWORD || 'change_me_user', role: 'user', syncsRemaining: 30, maxWebsites: 2 }
        ]);
