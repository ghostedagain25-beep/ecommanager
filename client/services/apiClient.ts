...
const handleApiResponse = async (response: Response) => {
  if (!response.ok) {
    ...
    const platform =
      response.url.includes('myshopify.com') || response.url.includes('shopify-proxy')
        ? 'Shopify'
        : 'WooCommerce';
...
export const apiFetch = async (
  ...
  if (website.platform === 'wordpress') {
    ...
    if (!website.url || !website.shopify_access_token) {
      throw new Error(
        `Shopify credentials are not configured for this website. Missing: ${!website.url ? 'URL' : ''} ${!website.shopify_access_token ? 'Access Token' : ''}`,
      );
    }
    ...
    if (pathOrUrl.startsWith('https://')) {
      ...
      apiPath = basePath.endsWith('.json')
        ? pathOrUrl
        : `${basePath}.json${queryParams ? `?${queryParams}` : ''}`;
    }
