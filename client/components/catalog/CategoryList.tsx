...
import { FolderSearchIcon } from '../ui/icons';
...
const CategoryItem: React.FC<{
  ...
  return (
    <div>
      <div
        className={`flex items-center justify-between p-3 rounded-lg transition-colors ${isSelected ? 'bg-sky-900/60' : 'bg-gray-800 hover:bg-gray-700/50'}`}
      >
  ...
  if (categories.length === 0) {
    return (
      <div className="text-center py-10 bg-slate-900/50 rounded-lg border-2 border-dashed border-slate-700">
        <FolderSearchIcon className="w-12 h-12 mx-auto text-gray-500" />
        <h3 className="mt-2 text-lg font-medium text-white">No Categories Found</h3>
        <p className="mt-1 text-sm text-gray-400">
          Your search returned no results, or no categories exist yet.
        </p>
      </div>
    );
  }
