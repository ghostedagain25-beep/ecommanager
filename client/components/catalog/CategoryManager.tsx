...
export const CategoryManager: React.FC<{ isAdminView?: boolean }> = ({ isAdminView = false }) => {
  ...
  const [viewState, setViewState] = useState<ViewState>(
    isAdminView ? 'select_user' : 'show_categories',
  );
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      const fetchedCategories = await wooApi.fetchWooCommerceProductCategories(selectedWebsite);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const { deletedCount, errorDetails } = await wooApi.batchDeleteWooCommerceCategories(
        selectedWebsite,
    ...
    (categoryId: number) => {
      const getAllDescendantIds = (id: number): number[] => {
        ...
        return children.reduce(
          (acc, childId) => [...acc, childId, ...getAllDescendantIds(childId)],
          [] as number[],
        );
      };
      ...
      setSelectedCategoryIds((prevSelectedIds) => {
        ...
        if (isSelecting) {
          ...
          while (parentId && parentId !== 0) {
            ...
            const allSiblingsSelected = siblingIds.every((siblingId) =>
              newSelectedIds.has(siblingId),
            );
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400">
          {isAdminView
            ? `Managing categories for ${selectedUser?.username} on `
            : 'Managing your categories on '}
      ...
      {selectedWebsite.platform === 'shopify' ? (
        <div className="text-center py-10 bg-slate-900/50 rounded-lg">
          ...
          <h3 className="mt-2 text-lg font-medium text-white">Feature Not Available for Shopify</h3>
          <p className="mt-1 text-sm text-gray-400">
            Category Management for Shopify stores is not yet supported.
          </p>
        </div>
      ) : (
        <>
          ...
          <ConfirmationModal
            ...
            message={
              <>
                ...
                <strong>{selectedCategoryIds.size} selected categories</strong>? This action cannot
                be undone.
              </>
            }
