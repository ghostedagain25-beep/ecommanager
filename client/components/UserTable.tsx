          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div className="min-w-0">
                <p className="font-bold text-lg text-white break-all">{user.username}</p>
            ...
            <div className="space-y-2 text-sm border-t border-b border-slate-700 py-3 my-3">
              <div className="flex justify-between">
                ...
                <span
                  className={`font-mono px-2 py-1 rounded-md text-xs ${(user.websiteCount ?? 0) >= user.maxWebsites && user.role !== 'admin' ? 'bg-red-900 text-red-200' : 'bg-green-900 text-green-200'}`}
                >
              ...
              {user.role === 'user' && (
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => onManageWebsites(user)}
                    className="flex items-center text-gray-300 hover:text-white hover:bg-gray-700 px-3 py-1 rounded-md text-xs transition-colors"
                  >
            ...
            <div className="flex justify-end items-center gap-4 text-sm font-medium">
              ...
              <button onClick={() => onEdit(user)} className="text-sky-400 hover:text-sky-300">
                Edit
              </button>
              <button
          ...
          <tbody className="bg-gray-800 divide-y divide-gray-700">
            {users.map((user) => (
              <tr key={user.username} className="hover:bg-gray-700/50">
                ...
                <td
                  data-label="Role"
                  className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 capitalize"
                >
                  {user.role}
                </td>
                <td
                  data-label="Syncs Remaining"
                  className="px-6 py-4 whitespace-nowrap text-sm text-sky-300"
                >
                  {user.syncsRemaining}
                </td>
                <td
                ...
                <td data-label="Manage Sites" className="px-6 py-4 whitespace-nowrap text-sm">
                  {user.role === 'user' && (
                    <button
                      onClick={() => onManageWebsites(user)}
                      className="flex items-center text-gray-300 hover:text-white hover:bg-gray-700 px-3 py-1 rounded-md text-xs transition-colors"
