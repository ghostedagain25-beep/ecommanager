...
const StatCard: React.FC<{ label: string; value: string | number; emoji: string }> = ({
  label,
  value,
  emoji,
}) => (
...
const OrderStatusBadge: React.FC<{ status: string; platform: 'wordpress' | 'shopify' }> = ({
  status,
  platform,
}) => {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-medium ${colorClass}`}
    >
      {status.replace(/[-_]/g, ' ')}
    </span>
  );
...
export const OrderViewer: React.FC = () => {
  ...
  const [viewState, setViewState] = useState<ViewState>(
    user?.role === 'admin' ? 'select_user' : 'show_orders',
  );
    ...
    async (isLoadMore = false) => {
      ...
      try {
        if (selectedWebsite.platform === 'wordpress') {
          ...
          const newOrders = wooOrders.map((o) =>
            formatWooCommerceOrder(o, selectedWebsite.currency_symbol),
          );
          ...
          const { orders: shopifyOrders, nextUrl: newNextUrl } = await fetchShopifyOrders(
            selectedWebsite,
            fetchUrl,
          );
          const newOrders = shopifyOrders.map((o) =>
            formatShopifyOrder(o, selectedWebsite.currency_symbol),
          );
  ...
  const stats = useMemo(() => {
    ...
    const processingCount = source.filter((o) =>
      ['pending', 'processing', 'on-hold', 'unfulfilled', 'partial'].includes(o.status),
    ).length;
    const completedCount = source.filter((o) =>
    ...
    return [
      ...
      {
        label: 'Revenue Shown',
        value: `${selectedWebsite?.currency_symbol || '$'}${revenue.toFixed(2)}`,
        emoji: '💰',
      },
    ];
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex-grow">
              ...
              <h1 className="text-3xl font-bold text-white tracking-tight">
                Orders: <span className="text-sky-400">{selectedWebsite?.name}</span>
              </h1>
            </div>
            {websites.length > 1 && (
      ...
      <div className="animate-fade-in space-y-6">
        ...
        <div className="my-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search loaded orders by #, name, or status..."
              value={searchTerm}
        ...
        <div>
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="text-gray-100 text-right break-words">
                      {o.customerName || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-start gap-4">
              ...
              <thead className="bg-slate-800 text-gray-300 uppercase text-xs">
                <tr>
                  {['Order #', 'Customer', 'Date', 'Status', 'Total', 'Details'].map((h) => (
                    <th
                      key={h}
                      className={`px-4 py-3 ${h === 'Details' ? 'text-right' : 'text-left'}`}
                    >
                      {h}
                    </th>
                  ))}
                ...
                {filteredOrders.map((o) => (
                  <tr key={o.id} className="border-t border-slate-700 hover:bg-slate-800/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      >
                        {isLoadingDetails && selectedOrderDetails?.id === o.id ? (
          ...
          {isLoading && (
            <div className="text-center py-12">
              <SpinnerIcon className="w-8 h-8 mx-auto text-sky-400" />
            </div>
          )}
        ...
        <div className="flex justify-center items-center gap-4 pt-4">
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 bg-sky-600 text-white px-5 py-2 rounded-lg shadow hover:bg-sky-700 transition-colors disabled:bg-gray-600"
          >
            <RotateCwIcon className="w-4 h-4" />Refresh
          </button>
          <button
          ...
          >
            <ArrowDownIcon className="w-4 h-4" />Load More
          </button>
        </div>
        <OrderDetailModal
