...
const DesktopNavLink: React.FC<{
  ...
  >
    ...
    <span
      className={`transition-opacity duration-200 whitespace-nowrap ${isExpanded ? 'opacity-100' : 'opacity-0 hidden'}`}
    >
      {item.label}
    </span>
  </a>
...
const Navigation: React.FC<NavigationProps> = ({
  ...
  const getVisibleNavItems = () => {
    if (role === 'admin') {
      return {
        desktop: adminNavItems,
        mobilePrimary: adminNavItems.slice(0, 4),
        mobileMore: adminNavItems.slice(4),
      };
    }
  ...
  const {
    ...
    mobilePrimary: mobilePrimaryItems,
    mobileMore: mobileMoreItems,
  } = getVisibleNavItems();
      ...
      >
        <div className="flex flex-col h-full">
          <div className="p-3 flex items-center border-b border-slate-800 h-[61px] flex-shrink-0">
            <span
              className={`font-bold text-xl text-white whitespace-nowrap overflow-hidden transition-all duration-200 ${showFullSidebar ? 'w-auto opacity-100' : 'opacity-0 hidden'}`}
            >
              StockPro
            </span>
            <button
