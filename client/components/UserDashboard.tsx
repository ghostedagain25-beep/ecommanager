...
const UserDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      if (!user) return;
      try {
        const steps = await api.getWorkflowSteps();
        setWorkflowConfig(steps);
      } catch (error) {
        console.error('Failed to load workflow steps', error);
        setStatus({ state: 'error', message: 'Could not load workflow configuration.' });
      }
    };
    loadInitialData();
  }, [user]);
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      const jsonData = await processStockFiles(
        closingStockFile,
        itemDirectoryFile,
        workflowConfig,
        updateStep,
        ...
        <div className="animate-fade-in">
          <header className="text-center mb-8">
            ...
            <p className="text-gray-400 mt-1">
              Choose one of your configured sites to begin the update process.
            </p>
          </header>
          {websites.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {websites.map((site) => (
                <button
                  ...
                  className="bg-gray-800 p-6 rounded-lg text-left hover:bg-gray-700/80 border border-gray-700 hover:border-sky-500 transition-all transform hover:scale-105"
                >
      ...
      <div className="animate-fade-in">
        <header className="text-center mb-10">
          ...
          <p className="text-lg text-gray-400 mt-2">
            Automated Excel Data Transformation & WooCommerce Sync
          </p>
        </header>
        ...
        <div className="bg-gray-900/50 p-8 rounded-lg transition-all duration-500">
          <div className="p-4 mb-8 bg-sky-900/50 border border-sky-700 rounded-lg flex items-center justify-between">
            ...
            <button
              onClick={handleResetSiteSelection}
              className="text-sm px-3 py-1 bg-gray-600 hover:bg-gray-500 rounded-md transition-colors"
            >
              Change
            </button>
          </div>
            ...
            <>
              ...
              <div className="mt-8 flex flex-col items-center space-y-6">
                <ProcessButton
                  ...
                  disabled={
                    !closingStockFile || !itemDirectoryFile || status.state === 'processing'
                  }
                ...
                {status.state === 'error' && (
                  <button
                    onClick={handleReset}
                    className="px-4 py-2 text-sm bg-red-600 hover:bg-red-500 rounded-md transition-colors"
                  >
                    Try Again
                  </button>
                )}
