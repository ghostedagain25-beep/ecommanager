...
import {
  ...
  batchUpdateWooCommerceProducts,
...
export const WordPressSync: React.FC<WordPressSyncProps> = ({
  processedData,
  user,
  website,
  onSyncSuccess,
  ...
  const { url: apiUrl, consumerKey, consumerSecret } = website;
  ...
  const generatePreview = useCallback(async () => {
    if (!apiUrl || !consumerKey || !consumerSecret) {
      setStatus({ state: 'error', message: 'Website configuration is missing API credentials.' });
      return;
    }
    if (syncsRemaining <= 0) {
    ...
    try {
      ...
      onProgress(
        `Preparing to process ${totalProducts} products in ${totalBatches} batches for preview.`,
      );

      const allToUpdate: ProductSyncChange[] = [];
      const allUpToDate: WooCommerceProduct[] = [];
      ...
      for (let i = 0; i < totalProducts; i += SYNC_BATCH_SIZE) {
        ...
        setStatus({
          state: 'previewing',
          message: `Fetching data for batch ${batchNum}/${totalBatches}...`,
        });
        ...
        for (const item of chunk) {
          ...
          if (wooProduct) {
            ...
            if (wooProduct.regular_price !== newRegularPrice)
              changes.regular_price = { old: wooProduct.regular_price, new: newRegularPrice };
            if (wooProduct.sale_price !== newSalePrice)
              changes.sale_price = { old: wooProduct.sale_price, new: newSalePrice };
            if (wooProduct.stock_quantity !== newStock)
              changes.stock_quantity = { old: wooProduct.stock_quantity, new: newStock };
            ...
            if (needsUpdate) {
              ...
              allToUpdate.push({ ...wooProduct, changes });
  ...
  }, [apiUrl, consumerKey, consumerSecret, processedData, syncsRemaining, website]);

  const handleConfirmSync = useCallback(async () => {
    if (!previewData || !apiUrl || !consumerKey || !consumerSecret) return;
    ...
    try {
      ...
      if (previewData.updatePayload.length > 0) {
        ...
        // FIX: Cast updatePayload to WooCommerceUpdatePayload[] to satisfy the function's expected type.
        const { updatedCount, errorDetails } = await batchUpdateWooCommerceProducts(
          website,
          previewData.updatePayload as WooCommerceUpdatePayload[],
        );
    ...
    <div className="mt-12 animate-fade-in border-t-2 border-gray-700 pt-8">
      ...
      <div className="max-w-3xl mx-auto bg-gray-900/50 p-8 rounded-lg">
        ...
        <div className="mt-8 p-4 bg-yellow-900/50 border border-yellow-700 rounded-md text-sm text-yellow-300">
          ...
          <p className="mt-1">
            For the sync to work, your WordPress server must accept requests from this application
            (CORS policy). If you encounter a "Network request failed" error, this is the most
            likely cause.
          </p>
        </div>
          ...
          {syncsRemaining <= 0 && !isProcessing && (
            <div className="mt-4 p-4 bg-yellow-900/50 border border-yellow-700 rounded-md text-sm text-yellow-300 text-center animate-fade-in">
              <p>
                ...
                attempts remaining. Please contact an administrator to add more.
              </p>
            </div>
          )}
        </div>
          ...
          <div className="mt-4 p-4 border border-gray-700 rounded-lg text-center">
            ...
            <div className="mt-2 flex flex-wrap justify-center gap-x-8 gap-y-2 text-sm">
              <p className="text-green-400">
                <span className="font-bold text-lg">{result.updated}</span> Products Updated
              </p>
