...
import type { WooCommerceProduct, WooCommerceUpdatePayload, WooCommerceOrder, WooCommerceProductCategory } from '../types/woocommerce';
...
export const deleteWooCommerceProduct = async (
    ...
    const response = await apiFetch(website, `products/${productId}?force=true`, { method: 'DELETE' });
...
export const deleteWooCommerceCategory = async (
    ...
    const response = await apiFetch(website, `products/categories/${categoryId}?force=true`, { method: 'DELETE' });
    return response.json();
};
