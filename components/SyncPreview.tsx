...
import type { SyncPreviewData, ChangesObject, ProductSyncChange, ChangeDetail } from '../types/sync';
...
const SyncPreview: React.FC<SyncPreviewProps> = ({ previewData, onConfirm, onCancel, isSyncing, syncsRemaining, currencySymbol }) => {
    const [activeTab, setActiveTab] = useState<'update' | 'uptodate' | 'notfound'>('update');

    const { toUpdate, upToDate, notFound } = previewData;

    const renderTable = (data: (ProductSyncChange | WooCommerceProduct | { sku: string, name: string })[]) => {
        if (data.length === 0) {
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {data.map((item, index) => (
                            <tr key={'sku' in item ? item.sku : index} className="hover:bg-gray-700/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{'sku' in item ? item.sku : 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{'name' in item ? item.name : 'N/A'}</td>
