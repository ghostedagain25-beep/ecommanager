        ...
        <div className="animate-fade-in">
            ...
            <div className="sm:hidden space-y-4">
                 {currentRows.length > 0 ? currentRows.map((row, index) => (
                    <div key={index} className="w-full rounded-2xl bg-[#0d1b2a] p-4 shadow-md border border-slate-700 text-gray-200">
                        <div className="space-y-2 text-sm">
                            {columns.map((colName, colIndex) => (
                                <div key={colIndex} className="flex justify-between items-start gap-4 border-b border-slate-700 pb-2 last:border-b-0">
                                    <span className="font-medium text-gray-400 capitalize flex-shrink-0">{colName.replace(/_/g, ' ')}</span>
                                    <span className="text-gray-100 text-right break-all">{row[colIndex] === null ? <span className="italic text-gray-500">NULL</span> : String(row[colIndex])}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="text-center py-8 text-gray-500">
                        {rows.length === 0 ? 'This table is empty.' : `No results found for "${searchTerm}".`}
                    </div>
                )}
            </div>
                        ...
                        {currentRows.length > 0 ? currentRows.map((row, index) => (
                            <tr key={index} className="hover:bg-gray-700/50">
                                {row.map((cell, cellIndex) => (
                                    <td key={cellIndex} data-label={columns[cellIndex]} className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 max-w-md truncate">
            ...
            <div className="flex items-center justify-between pt-4 px-2">
             ...
             <p className="text-center text-xs text-gray-500 mt-4">
                ...
                {searchTerm && filteredData.length !== rows.length && ` (Filtered from ${rows.length} total)`}
             </p>
        </div>
    );
};