...
import { ShopifyIcon, SpinnerIcon, CheckCircleIcon, ExclamationIcon, InfoIcon, EyeIcon } from '../ui/icons';
...
export const ShopifySync: React.FC<ShopifySyncProps> = ({ processedData, user, website, onSyncSuccess }) => {
  ...
  const generatePreview = useCallback(async () => {
    ...
    try {
      ...
      setStatus({ state: 'previewing', message: `Fetching products from Shopify... This may take a moment.` });
      ...
      for (const item of processedData) {
        ...
        if (shopifyVariant) {
            ...
            const newCompareAtPriceForUpdate = newRegularPriceNum > 0 ? String(newRegularPriceNum) : null;
            ...
            const oldCompareAtPriceNum = shopifyVariant.compare_at_price ? parseFloat(shopifyVariant.compare_at_price) : 0;
            if (oldCompareAtPriceNum !== newRegularPriceNum) {
                changes.compare_at_price = { old: shopifyVariant.compare_at_price, new: newCompareAtPriceForUpdate };
            }
            ...
            if (needsUpdate) {
                ...
                allSyncDetailsForReport.push({ sku, product_name: productName, status: 'updated', changes_json: JSON.stringify(changes) });
            } else {
                allUpToDate.push({ sku, name: productName });
                allSyncDetailsForReport.push({ sku, product_name: productName, status: 'up_to_date', changes_json: '{}' });
            }
        } else {
  ...
  const handleConfirmSync = useCallback(async () => {
    ...
    try {
        ...
        const { updatedCount, errorDetails } = await batchUpdateShopifyVariants(website, previewData.updatePayload as ShopifyVariantUpdatePayload[]);
        ...
        if (errorDetails.length > 0) {
            ...
            setStatus({ state: 'success', message: `Sync complete with ${errorDetails.length} error(s).` });
        } else {
      ...
      <div className="max-w-3xl mx-auto bg-gray-900/50 p-8 rounded-lg">
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 p-2 text-gray-200 bg-gray-700/50 border border-gray-600 rounded-md">{website.url}.myshopify.com</p>
          </div>
        </div>
        ...
        <div className="mt-6">
            ...
            {syncsRemaining <= 0 && !isProcessing && (
              <div className="mt-4 p-4 bg-yellow-900/50 border border-yellow-700 rounded-md text-sm text-yellow-300 text-center animate-fade-in">
                <p><span className="font-bold">Out of Syncs:</span> You have no synchronization attempts remaining.</p>
              </div>
            )}
        </div>
        ...
        {result && status.state === 'success' && (
          <div className="mt-4 p-4 border border-gray-700 rounded-lg text-center">
            ...
            <div className="mt-2 flex flex-wrap justify-center gap-x-8 gap-y-2 text-sm">
                <p className="text-green-400"><span className="font-bold text-lg">{result.updated}</span> Variants Updated</p>
