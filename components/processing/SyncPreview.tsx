...
import type { SyncPreviewData, ChangesObject, PreviewableItem, ChangeDetail } from '../../types/sync';
import { SpinnerIcon, SyncIcon, CancelIcon, EyeIcon } from '../ui/icons';

const ChangesDisplay: React.FC<{ changes: ChangesObject, currencySymbol: string }> = ({ changes, currencySymbol }) => {
...
export const SyncPreview: React.FC<SyncPreviewProps> = ({ previewData, onConfirm, onCancel, isSyncing, syncsRemaining, currencySymbol }) => {
    const [activeTab, setActiveTab] = useState<'update' | 'uptodate' | 'notfound'>('update');

    const { toUpdate, upToDate, notFound } = previewData;

    const renderTable = (data: (PreviewableItem | { sku: string, name: string })[]) => {
        if (data.length === 0) {
             ...
             <div className="overflow-x-auto rounded-lg border border-gray-700 bg-gray-900/50 max-h-96">
                <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {data.map((item, index) => (
                            <tr key={item.sku || index} className="hover:bg-gray-700/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{item.sku || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{item.name || 'N/A'}</td>
                                {activeTab === 'update' && 'changes' in item && (
                                    <td className="px-6 py-4 text-sm text-gray-300"><ChangesDisplay changes={item.changes} currencySymbol={currencySymbol} /></td>
                                )}
                            </tr>
                        ))}
                    </tbody>
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'update': return renderTable(toUpdate);
            case 'uptodate': return renderTable(upToDate);
            case 'notfound': return renderTable(notFound.map(n => ({ ...n, name: n.product_name })));
            default: return null;
        }
    };

    const TabButton: React.FC<{ tabKey: 'update' | 'uptodate' | 'notfound', label: string, count: number }> = ({ tabKey, label, count }) => {
        ...
        return (
            ...
            >
                {label} <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${isActive ? 'bg-sky-200 text-sky-800' : 'bg-gray-600 text-gray-200'}`}>{count}</span>
            </button>
        )
    }
    ...
    return (
        <div className="animate-fade-in">
            <header className="text-center mb-6">
                ...
                <p className="text-gray-400 mt-2">Review the proposed changes before applying them to your store.</p>
            </header>
            ...
            <div className="bg-gray-900/50 p-6 rounded-lg">
                ...
                <div className="mt-6 pt-6 border-t border-gray-700 flex justify-end items-center gap-4">
                    <button onClick={onCancel} disabled={isSyncing} className="flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded-md transition-colors disabled:opacity-50">
                 ...
                 {toUpdate.length === 0 && (
                    <p className="text-center text-sm text-green-400 mt-4">All products are already up-to-date. No action needed.</p>
                )}
                 {syncsRemaining <= 0 && toUpdate.length > 0 && (
                    <p className="text-center text-sm text-red-400 mt-4">Cannot sync: You have no syncs remaining. Please contact an administrator.</p>
                )}
            </div>
        </div>
    );
