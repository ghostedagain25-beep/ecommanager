        ...
        <div className="animate-fade-in">
            ...
            <div className="sm:hidden space-y-4">
                {currentRows.map((row, index) => (
                    <div key={`${row.SKU}-${index}`} className="w-full rounded-2xl bg-[#0d1b2a] p-4 shadow-md border border-slate-700 text-gray-200">
                        <div className="flex justify-between items-start mb-3">
                            <div>
                                <p className="text-xs text-gray-400">SKU</p>
                                <p className="font-bold text-white">{row.SKU}</p>
                            </div>
                            ...
                            {Object.entries(row).map(([key, value]) => {
                                ...
                                return (
                                    <div key={key} className="flex justify-between">
                                        <span className="font-medium text-gray-400 capitalize">{key.toLowerCase().replace(/_/g, ' ')}</span>
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {currentRows.length > 0 ? currentRows.map((row, index) => (
                            <tr key={`${row.SKU}-${index}`} className="hover:bg-gray-700/50">
                                {headers.map(key => (
                                    <td key={key} data-label={key} className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                        {row[key]}
                                    </td>
                                ))}
                            </tr>
            ...
            <div className="flex items-center justify-between pt-4 px-2">
             ...
             <p className="text-center text-xs text-gray-500 mt-4">
                ...
                {searchTerm && filteredData.length !== data.length && ` (Filtered from ${data.length} total)`}
             </p>
        </div>
    );
};