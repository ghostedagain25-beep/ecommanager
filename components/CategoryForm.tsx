...
const CategoryForm: React.FC<CategoryFormProps> = ({ category, allCategories, onSave, onCancel, isSaving }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 bg-slate-800 border border-slate-700 rounded-lg space-y-6 animate-fade-in">
            <h2 className="text-xl font-semibold text-white">{isEditing ? `Editing: ${category?.name}` : 'Create New Category'}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-300">Parent Category</label>
                     <select name="parent" value={formData.parent} onChange={handleChange} className="mt-1 w-full block py-2 px-3 text-white bg-gray-700 border border-gray-600 rounded-md focus:ring-sky-500 focus:border-sky-500">
                ...
                <div className="md:col-span-2">
                    ...
                    <textarea name="description" value={formData.description} onChange={handleChange} rows={4} className="mt-1 w-full block py-2 px-3 text-white bg-gray-700 border border-gray-600 rounded-md focus:ring-sky-500 focus:border-sky-500" />
                </div>
            </div>
