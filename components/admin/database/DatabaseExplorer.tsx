...
import { SpinnerIcon, TableIcon, ExclamationIcon, InfoIcon, SearchIcon, CancelIcon } from '../../ui/icons';
...
const DatabaseExplorer: React.FC = () => {
    ...
    const handleSelectTable = useCallback(async (tableName: string) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : `Failed to load data for table "${tableName}".`);
        } finally {
    ...
    const renderGlobalSearchResults = () => {
        ...
        const totalMatches = tablesWithResults.reduce((acc, tableName) => acc + globalSearchResults[tableName].rows.length, 0);
        ...
        <div className="animate-fade-in">
            ...
            <div className="mb-6 p-4 bg-gray-900/50 rounded-lg border border-gray-700">
                ...
                <form onSubmit={handleGlobalSearch} className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button type="submit" disabled={isGlobalSearching} className="flex items-center justify-center px-4 py-2 bg-sky-600 hover:bg-sky-700 rounded-md transition-colors disabled:bg-gray-600">
                        {isGlobalSearching ? <SpinnerIcon className="w-5 h-5" /> : <SearchIcon className="w-5 h-5" />}
                        <span className="ml-2">Search</span>
                    </button>
                    {(globalSearchResults || globalSearchTerm) && (
                        <button type="button" onClick={clearGlobalSearch} className="flex items-center justify-center px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded-md transition-colors">
            ...
            {error && (
                 <div className="flex items-center p-4 mb-4 text-sm text-red-300 bg-red-900/50 rounded-lg" role="alert">
