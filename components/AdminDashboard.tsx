...
import AdminUserWebsites from './AdminUserWebsites';
...
import { SpinnerIcon, ExclamationIcon, UsersIcon, SearchIcon, ClipboardListIcon, InfoIcon } from './icons';
...
const UserManagement: React.FC = () => {
    ...
    const handleLoginAs = (username: string) => {
        if (window.confirm(`You are about to log in as "${username}". This will log you out of your admin account. Proceed?`)) {
            try {
    ...
    if (error) {
         return <div className="flex items-center p-4 text-sm text-red-300 bg-red-900/50 rounded-lg" role="alert"><ExclamationIcon className="w-5 h-5 mr-3"/>{error}</div>;
    }

    const renderUserManagementContent = () => {
        switch (view) {
            case 'form':
                return <UserForm user={selectedUser} onSave={handleSave} onCancel={handleCancel} />;
            case 'websites':
                if (!userForWebsites) return null;
                return <AdminUserWebsites user={userForWebsites} onBack={handleBackFromWebsites} />;
            case 'menu_settings':
                 ...
                 return (
                     <>
                        <header className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={handleAddNew} className="flex items-center px-4 py-2 bg-sky-600 hover:bg-sky-700 rounded-md transition-colors">
...
const AdminDashboard: React.FC = () => {
    ...
    const PushHistoryInline: React.FC = () => {
        ...
        const handleViewDetails = useCallback(async (summary: any) => {
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                        <thead className="bg-gray-700/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Processed</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Pushed</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Errors</th>
                                <th className="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase tracking-wider">Details</th>
                            </tr>
                        </thead>
                            ...
                            {summaries.map((s: any) => (
                                <React.Fragment key={s._id}>
                                    <tr className="hover:bg-gray-700/50">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{s.user_username}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{new Date(s.push_timestamp).toLocaleString()}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{s.type}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-sky-300 text-center">{s.total_processed}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-green-400 text-center">{s.total_pushed}</td>
                                        <td className={`px-6 py-4 whitespace-nowrap text-sm text-center ${s.total_errors > 0 ? 'text-red-400' : 'text-gray-400'}`}>{s.total_errors}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button onClick={() => handleViewDetails(s)} className="text-sky-400 hover:text-sky-300 disabled:text-gray-500 disabled:cursor-wait" disabled={isLoadingDetails && selected?._id === s._id}>
                                                ...
                                                {details && (
                                                    <div className="overflow-x-auto">
                                                        <table className="min-w-full divide-y divide-gray-700">
                                                            <thead className="bg-gray-700/50">
                                                                <tr>
                                                                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">SKU</th>
                                                                    ...
                                                                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Changes</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody className="bg-gray-800 divide-y divide-gray-700">
                                                                {details.map((d: any, idx: number) => (
                                                                    <tr key={idx}>
                                                                        ...
                                                                        <td className="px-4 py-2 text-sm text-gray-300">{d.product_name || '-'}</td>
                                                                        <td className="px-4 py-2 text-sm text-gray-300">{d.action}</td>
                                                                        <td className="px-4 py-2 text-sm text-gray-300">{d.status}</td>
                                                                        <td className="px-4 py-2 text-sm text-gray-300 whitespace-pre-wrap break-all">{d.changes_json || '-'}</td>
                                                                    </tr>
                                                                ))}
                                                            </tbody>
                                                        </table>
