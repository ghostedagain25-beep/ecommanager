...
import { SpinnerIcon, ExclamationIcon, UserIcon, ArrowLeftIcon, WordPressIcon, SearchIcon, PlusCircleIcon, CheckCircleIcon, Trash2Icon, ShopifyIcon } from '../ui/icons';
...
export const ProductManager: React.FC<ProductManagerProps> = ({ isAdminView = false }) => {
    ...
    const [viewState, setViewState] = useState<ViewState>(isAdminView ? 'select_user' : 'show_products');
    ...
    const fetchProducts = useCallback(async (pageNum: number, search: string, categoryId: string, refreshing = false) => {
        if (!selectedWebsite || selectedWebsite.platform !== 'wordpress') {
            setProducts([]);
            setIsLoading(false);
        ...
        try {
            ...
            setCanLoadMore(newProducts.length === 10);
        } catch (err) {
            ...
            if (selectedWebsite.platform === 'wordpress') {
                const fetchCats = async () => {
                    try {
                        ...
                        setError("Could not fetch categories. Check credentials and CORS settings.");
                    }
                };
                fetchCats();
            }
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            const { deletedCount, errorDetails } = await wooApi.batchDeleteWooCommerceProducts(selectedWebsite, Array.from(selectedProductIds));
            ...
            <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
                <div className="w-full max-w-2xl">
                     <ProductForm 
                        ...
                        onCancel={() => { setIsFormVisible(false); setProductToEdit(null); setError(null); }}
                    />
                    {error && <div className="mt-4 flex items-center p-4 text-sm text-red-300 bg-red-900/50 rounded-lg"><ExclamationIcon className="w-5 h-5 mr-3"/>{error}</div>}
                </div>
            </div>
        );
    }
    ...
    if (viewState === 'show_products') {
        if (isLoading && products.length === 0 && !selectedWebsite) {
    ...
    const allOnPageSelected = products.length > 0 && products.every(p => selectedProductIds.has(p.id));
         ...
         <div className="animate-fade-in space-y-6">
            <div>
                ...
                <p className="text-gray-400">
                    {isAdminView ? `Managing products for ${selectedUser?.username} on ` : 'Managing your products on '}
             ...
             <div className="flex flex-col md:flex-row justify-between items-center gap-4 p-4 bg-gray-900/50 rounded-lg">
                <form onSubmit={handleSearch} className="flex-grow flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <div className="relative flex-grow">
                        <span className="absolute inset-y-0 left-0 flex items-center pl-3"><SearchIcon className="w-5 h-5 text-gray-400" /></span>
                        <input type="text" placeholder={`Search products...`} className="w-full pl-10 pr-4 py-2 text-white bg-gray-700 border border-gray-600 rounded-md focus:ring-sky-500 focus:border-sky-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                     <select value={selectedCategoryId} onChange={handleCategoryChange} className="bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-sky-500 focus:border-sky-500">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                    {websites.length > 1 && (
                     ...
                     <button onClick={() => { setProductToEdit(null); setIsFormVisible(true); }} className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md text-white w-full justify-center md:w-auto">
            ...
            <div className="sm:hidden space-y-4">
                {products.map(p => (
                    <div key={p.id} className={`w-full rounded-2xl p-4 shadow-md border text-gray-200 transition-colors ${selectedProductIds.has(p.id) ? "bg-sky-900/60 border-sky-700" : "bg-[#0d1b2a] border-slate-700"}`}>
                        <div className="flex gap-4">
                            <input type="checkbox" onChange={() => handleSelectProduct(p.id)} checked={selectedProductIds.has(p.id)} className="mt-1 rounded border-gray-500 bg-gray-700 text-sky-500 focus:ring-sky-500" />
                            <img src={p.images?.[0]?.src || 'https://via.placeholder.com/50'} alt={p.name} className="w-16 h-16 object-cover rounded-md bg-gray-700 flex-shrink-0" />
                            <div className="flex-1 min-w-0">
                                <p className="font-bold text-white break-words">{p.name}</p>
                                <p className="text-sm text-gray-400 break-all">SKU: {p.sku || 'N/A'}</p>
                                <p className="text-xs text-gray-500 break-words">{p.categories?.map(c => c.name).join(', ') || 'Uncategorized'}</p>
                            </div>
                        </div>
                        <div className="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
                            <div>
                                <p className="text-xs text-gray-400">Price</p>
                                <p className="font-semibold text-white">{selectedWebsite?.currency_symbol}{p.regular_price}</p>
                            </div>
                            <div>
                                <p className="text-xs text-gray-400">Sale</p>
                                <p className="font-semibold text-green-400">{selectedWebsite?.currency_symbol}{p.sale_price}</p>
                            </div>
                            <div>
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-700 flex justify-end gap-4">
                            <button onClick={() => {setProductToEdit(p); setIsFormVisible(true);}} className="text-sky-400 hover:text-sky-300 text-sm font-medium">Edit</button>
                            <button onClick={() => setProductToDelete(p)} className="text-red-400 hover:text-red-300 text-sm font-medium">Delete</button>
                        </div>
                    </div>
                ))}
            </div>
                    ...
                    <thead className="bg-gray-700/50">
                        <tr>
                            <th scope="col" className="px-6 py-3">
                                <input type="checkbox" onChange={handleSelectAll} checked={allOnPageSelected} className="rounded border-gray-500 bg-gray-700 text-sky-500 focus:ring-sky-500" />
                            </th>
                            {['Image', 'SKU', 'Name', 'Categories', 'Regular Price', 'Sale Price', 'Stock', 'Actions'].map(h => <th key={h} className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">{h}</th>)}
                        </tr>
                    </thead>
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {products.map(p => (
                            <tr key={p.id} className={selectedProductIds.has(p.id) ? "bg-sky-900/60" : "hover:bg-gray-700/50"}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <input type="checkbox" onChange={() => handleSelectProduct(p.id)} checked={selectedProductIds.has(p.id)} className="rounded border-gray-500 bg-gray-700 text-sky-500 focus:ring-sky-500" />
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <img 
                                        src={p.images?.[0]?.src || 'https://via.placeholder.com/50'} 
                                        alt={p.name} 
                                        className="w-12 h-12 object-cover rounded-md bg-gray-700"
                                    />
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{p.sku}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{p.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{p.categories?.map(c => c.name).join(', ') || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{selectedWebsite?.currency_symbol}{p.regular_price}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-green-400">{selectedWebsite?.currency_symbol}{p.sale_price}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-sky-300">{p.stock_quantity ?? 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => {setProductToEdit(p); setIsFormVisible(true);}} className="text-sky-400 hover:text-sky-300 mr-4">Edit</button>
                                    <button onClick={() => setProductToDelete(p)} className="text-red-400 hover:text-red-300">Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <ConfirmationModal
                ...
                message={<>Are you sure you want to permanently delete the product <strong>{productToDelete?.name}</strong>? This action cannot be undone.</>}
                confirmText="Delete"
            ...
            <ConfirmationModal
                ...
                message={<>Are you sure you want to permanently delete <strong>{selectedProductIds.size} selected products</strong>? This action cannot be undone.</>}
