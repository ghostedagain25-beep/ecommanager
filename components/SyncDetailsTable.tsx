...
import type { SyncDetail, ChangesObject, ChangeDetail } from '../types/sync';
                ...
                {Object.entries(changes).map(([key, value]) => {
                    ...
                    return (
                        <li key={key} className="flex flex-wrap items-baseline">
                            <span className="font-semibold text-gray-300 capitalize mr-2">{key.replace(/_/g, ' ')}:</span>
                            <span className="text-red-400 line-through mr-1 break-all">{String(detail.old ?? 'N/A')}</span>
...
const SyncDetailsTable: React.FC<SyncDetailsTableProps> = ({ details }) => {
    ...
    const requestSort = (key: keyof SyncDetail) => {
     ...
     const getSortIcon = (key: keyof SyncDetail) => {
        ...
        if (sortConfig.direction === 'ascending') return <SortAscIcon className="w-4 h-4 text-sky-400" />;
    ...
    return (
        <div className="animate-fade-in">
            <h3 className="text-xl font-semibold text-white mb-4">Detailed Sync Report</h3>
             <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4 p-4 bg-gray-900/50 rounded-lg">
                <div className="relative w-full md:w-auto md:flex-grow">
                    ...
                    <input type="text" placeholder={`Search ${details.length} records...`} className="w-full pl-10 pr-4 py-2 text-white bg-gray-700 border border-gray-600 rounded-md focus:ring-sky-500 focus:border-sky-500" onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} value={searchTerm} />
                </div>
            </div>
            ...
            <div className="sm:hidden space-y-4">
                {currentRows.map(row => (
                    <div key={row.id} className="w-full rounded-2xl bg-[#0d1b2a] p-4 shadow-md border border-slate-700 text-gray-200">
             ...
             <div className="hidden sm:block overflow-x-auto rounded-lg border border-gray-700 bg-gray-900/50">
                <table className="min-w-full divide-y divide-gray-700">
                    <thead className="bg-gray-700/50">
                        <tr>
                            {headers.map((key) => (
                                <th key={key} scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-700" onClick={() => requestSort(key)}>
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {currentRows.length > 0 ? currentRows.map((row) => (
                            <tr key={row.id} className="hover:bg-gray-700/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{row.sku}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{row.product_name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300"><StatusBadge status={row.status} /></td>
                                <td className="px-6 py-4 text-sm text-gray-300"><ChangesDisplay changesJson={row.changes_json} /></td>
                            </tr>
                        )) : (
                             <tr>
            ...
            <div className="flex items-center justify-between pt-4 px-2">
             ...
             <p className="text-center text-xs text-gray-500 mt-4">
                ...
                {searchTerm && filteredData.length !== details.length && ` (Filtered from ${details.length} total)`}
             </p>
        </div>
    );
};
