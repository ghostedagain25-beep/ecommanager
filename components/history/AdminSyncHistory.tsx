...
import { SpinnerIcon, InfoIcon, ExclamationIcon, ClipboardListIcon } from '../ui/icons';
...
const AdminSyncHistory: React.FC = () => {
    ...
    const handleViewDetails = useCallback(async (summary: SyncHistorySummary) => {
        ...
        <div className="animate-fade-in">
            ...
            <div className="sm:hidden space-y-4">
                {summaries.map(summary => (
                    <div key={summary.id} className="w-full rounded-2xl bg-[#0d1b2a] p-4 shadow-md border border-slate-700 text-gray-200">
                        <div className="flex justify-between items-start mb-3">
                            <div className="min-w-0">
                                <p className="font-bold text-lg text-white break-all">{summary.user_username}</p>
                                <p className="text-sm text-gray-400">{new Date(summary.sync_timestamp).toLocaleString()}</p>
                            </div>
                            <button onClick={() => handleViewDetails(summary)} className="text-sky-400 hover:text-sky-300 disabled:text-gray-500 disabled:cursor-wait p-2 flex-shrink-0 ml-2" disabled={isLoadingDetails && selectedSync?.id === summary.id}>
                                {isLoadingDetails && selectedSync?.id === summary.id ? <SpinnerIcon className="w-5 h-5"/> : <ClipboardListIcon className="w-5 h-5"/>}
                            </button>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center text-sm border-t border-b border-slate-700 py-3">
                            ...
                            <div>
                                ...
                                <p className={`font-semibold ${summary.total_errors > 0 ? 'text-red-400' : 'text-gray-400'}`}>{summary.total_errors}</p>
                            </div>
                        </div>
                ...
                <table className="min-w-full divide-y divide-gray-700">
                    <thead className="bg-gray-700/50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Processed</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Updated</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Errors</th>
                            <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase tracking-wider">Details</th>
                        </tr>
                    </thead>
                        ...
                        {summaries.map(summary => (
                            <React.Fragment key={summary.id}>
                                <tr className="hover:bg-gray-700/50">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{summary.user_username}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{new Date(summary.sync_timestamp).toLocaleString()}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-sky-300 text-center">{summary.total_processed}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-green-400 text-center">{summary.total_updated}</td>
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-center ${summary.total_errors > 0 ? 'text-red-400' : 'text-gray-400'}`}>{summary.total_errors}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button onClick={() => handleViewDetails(summary)} className="text-sky-400 hover:text-sky-300 disabled:text-gray-500 disabled:cursor-wait" disabled={isLoadingDetails && selectedSync?.id === summary.id}>
