...
import { SpinnerIcon, ExclamationIcon, UserIcon, ArrowLeftIcon, WordPressIcon, SearchIcon, PlusCircleIcon, CheckCircleIcon, Trash2Icon } from './icons';
             ...
             <div className="flex flex-col md:flex-row justify-between items-center gap-4 p-4 bg-gray-900/50 rounded-lg">
                <form onSubmit={handleSearch} className="flex-grow flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <div className="relative flex-grow">
                     ...
                     <select value={selectedCategoryId} onChange={handleCategoryChange} className="bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-sky-500 focus:border-sky-500">
                        ...
                        {categories.map((cat, index) => <option key={cat.id || `category-${index}`} value={cat.id}>{cat.name}</option>)}
                    </select>
                </form>
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                    {websites.length > 1 && (
                        ...
                        >
                            {websites.map((site, index) => <option key={site.id || `website-${index}`} value={site.id}>{site.name}</option>)}
                        </select>
                    )}
                    ...
                    <div key={p.id} className={`w-full rounded-2xl p-4 shadow-md border text-gray-200 transition-colors ${selectedProductIds.has(p.id) ? "bg-sky-900/60 border-sky-700" : "bg-[#0d1b2a] border-slate-700"}`}>
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 break-words">{p.categories?.map((c, index) => c.name).join(', ') || 'Uncategorized'}</p>
                            </div>
                        </div>
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {products.map(p => (
                            <tr key={p.id} className={selectedProductIds.has(p.id) ? "bg-sky-900/60" : "hover:bg-gray-700/50"}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 truncate max-w-xs">{p.categories?.map((c, index) => c.name).join(', ') || 'N/A'}</td>
